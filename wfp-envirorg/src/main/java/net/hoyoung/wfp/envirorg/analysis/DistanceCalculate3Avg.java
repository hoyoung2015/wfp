...
public class DistanceCalculate3Avg {
    ...
    private List<Map<String,Object>> hporgs;
    ...
    public DistanceCalculate3Avg() {
        ...
        hporgs = jdbcTemplate.queryForList("SELECT id as hporg_id,name,pos_x,pos_y FROM hporg");
    }
    public List<Map<String,Object>> calculate(){
        jdbcTemplate.execute("DROP TABLE if EXISTS `com_hporg_dis_avg`");
        jdbcTemplate.execute("create table com_hporg_dis_avg(stock_code VARCHAR(10),avg_dis int,PRIMARY KEY (stock_code))");
        int count = 1;
        for (Map<String,Object> com : companies){
            int dis = avgDis(com);
            jdbcTemplate.update("INSERT INTO com_hporg_dis_avg(stock_code,avg_dis) VALUES (?,?)",com.get("stock_code"),dis);
    ...
    private int avgDis(Map<String,Object> com){
        Integer tmp = jdbcTemplate.queryForObject("select avg(dis) from (SELECT dis FROM com_hporg_dis WHERE stock_code=? ORDER BY dis asc limit 3) t",
