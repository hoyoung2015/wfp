...
public class DownloadFile extends Thread {
	...
	public void run() {
		while (endPos > startPos && !isDownloadOver) {
			...
			try {
				...
				int length = -1;
				logger.debug("#start#Thread: " + threadId + ", startPos: " + startPos + ", endPos: " + endPos);
				long tmp = startPos;
				...
				logger.debug("#over#Thread: " + threadId + ", startPos: " + startPos + ", endPos: " + endPos);
				logger.debug("Thread " + threadId + " is execute over!");
		...
		if (endPos < startPos && !isDownloadOver) {
			logger.debug("Thread " + threadId + " startPos > endPos, not need download file !");
			this.isDownloadOver = true;
		}
		if (endPos == startPos && !isDownloadOver) {
			logger.debug("Thread " + threadId + " startPos = endPos, not need download file !");
	...
	public static void printHeader(URLConnection conn) {
		...
		while (true) {
			...
			i++;
			if (header != null) {
				logger.debug(header + ":" + conn.getHeaderField(i));
			} else {
				break;
			}
		}
