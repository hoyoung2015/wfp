...
import us.codecraft.webmagic.downloader.selenium.FuckSeleniumDownloader;
 ...
 */
@ThreadSafe
public class FuckDownloader extends AbstractDownloader {
	
	private FuckSeleniumDownloader seleniumDownloader = new FuckSeleniumDownloader("/Users/baidu/local/bin/chromedriver");
	...
	public Page download(Request request, Task task) {
		...
		try {
			...
			if (statusAccept(acceptStatCode, statusCode)) {
				...
				if("xml".equals(contentTypeValue)){
				...
				if (page.getContentLength() == -1 || Pattern.matches("html", contentTypeValue)) {// html
					...
					// 渲染
					Page fuckPage = seleniumDownloader.download(request, task);
					
					
					
					String content = getContent(charset, httpResponse,fuckPage.getRawText());
	...
	protected String getContent(String charset, HttpResponse httpResponse,String xuanranHtml) throws IOException {
		if (charset == null) {
			byte[] contentBytes = xuanranHtml.getBytes();
			...
			return IOUtils.toString(xuanranHtml.getBytes(), charset);
		}
	}
