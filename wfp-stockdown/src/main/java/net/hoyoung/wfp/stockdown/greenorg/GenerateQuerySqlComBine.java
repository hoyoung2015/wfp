...
//        int[] diss = {100};
        ...
        String fieldTravelTemp = " ,if(t2_[dis].green_org_num is null,0,t2_[dis].green_org_num) as travel_[dis] ";
        String joinTravelTemp = " left join (select stock_code,count(*) as green_org_num from com_org where distance2<=[dis]*1000 group by stock_code) t2_[dis] on a.stock_code=t2_[dis].stock_code ";
        for (int i = 0; i < diss.length; i++) {
