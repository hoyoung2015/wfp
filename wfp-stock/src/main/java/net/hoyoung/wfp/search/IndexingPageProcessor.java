...
public class IndexingPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		if (StringUtils.isEmpty(src)) {
			src = page.getHtml().$("#content_left > div.c-border.c-row.site_tip > div > p:nth-child(1) > b", "text")
					.get();
		}
		if (StringUtils.isEmpty(src)) {
			return;
		...
		page.putField("document", new Document("stockCode", page.getRequest().getExtra("stockCode")).append(COLUNM_NAME,
				Integer.valueOf(src)));
	}
